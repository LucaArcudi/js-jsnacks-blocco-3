...
function getNewElement (content, elementTag){
    const newEl = document.createElement(elementTag);
    newEl.innerText = content;
    newEl.classList.add(classesToAdd);
    return newEl;
}
 
let divRedOddElement = document.getElementById("text-red");
let divGreenEvenElement = document.getElementById("text-green");

for (let i = 0; i < array.length; i++){
    if (array[i] % 2 === 0){
        divRedOddElement.appendChild(getNewElement(array[i]));
    } else {
        divGreenEvenElement.appendChild(getNewElement(array[i]));
    }
}
